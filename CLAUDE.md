...
- **tools-сервисы** (profile `tools`, образ = builder-стадия): `migrate` (`drizzle-kit migrate`), `seed`, `seed-site` (site_settings+faq, idempotent, без ALLOW_PROD_SEED), `seed-marketplace-links` (sku-ссылки МП из снапшота, idempotent, трогает ТОЛЬКО skus.marketplaces), `push-media`. Запуск: `docker compose -f docker-compose.prod.yml --profile tools run --rm <name>`. **При деплое с новыми миграциями/кодом собирать `--no-cache`** — `up --build` может взять слой `COPY . .` из build cache и не подтянуть свежие файлы. **Грабля (2026-06-10):** `build --no-cache` без `--profile tools` НЕ пересобирает образ `migrate` — старый образ бежит по старым файлам миграций и пишет «applied successfully», ничего не применив. Перед migrate: `--profile tools build --no-cache migrate`; проверка — журнал `drizzle.__drizzle_migrations` и фактические колонки.
...
- `/catalog/[slug]` — карточка товара: галерея, название, цена, описание, tech specs, выбор цвета+размера → «В корзину» (под ним подпись «* положите товары в корзину…»). Ниже отдельный блок «Купить на маркетплейсе»: кнопки Kaspi/Ozon показываются всегда, если товар вообще представлен на МП (product-level или любой SKU); без активной ссылки для текущего выбора кнопка серая некликабельная (`mp-disabled-*`) — **при выбранном размере ведут на карточку этого SKU** (marketplace-sku-links: `effective = {...product.marketplaces, ...sku.marketplaces}`, без размера / без sku-ссылки — продуктовый fallback); под кнопками подпись «* перейдите по ссылкам…», в конце «Возврат 30 дней.» (строки географии «Алматы — …» больше НЕТ); coming_soon → «Узнать о наличии» (wa.me). **Наличие (cart-inventory, done):** точка-светофор справа от цены по `available = stockQty − reservedQty` (зелёная ≥10 / оранжевая 3–9 / красная 1–2, пороги — константы в `@/core/inventory/client`; цифры покупателю не показываются); распроданный размер перечёркнут, но кликабелен → CTA «Узнать о поступлении» (wa.me с товаром/цветом/размером)
...
- **Заказы (cart-whatsapp, done):** таблицы `orders` (+`number` serial unique — человеческий №) и `order_items` (снапшоты имени/цены). Создание — ТОЛЬКО `createOrder` из `@/core/orders` (zod 1..30 позиций, qty 1..20; цены из БД; недоступные SKU и `available < qty` → отказ без записи, резерв при создании НЕ ставится). Статусы: `pending`(«Новый»)→`confirmed`→`done`/`cancelled`, меняются вручную в `/admin/orders`. WhatsApp-номер заказов — `site_settings.whatsapp` (админка «Настройки сайта», wa.me-ссылка в футере). `marketplaces` (Kaspi/Ozon) редактируются в `ProductForm` на табе «Маркетплейсы».
- **Ссылки МП per-SKU (marketplace-sku-links, done):** `skus.marketplaces` jsonb (миграция 0006) — у каждого размера свои ссылки Ozon/Kaspi (Kaspi не группирует варианты). Редактор — таб «Маркетплейсы» в `ProductForm` (продуктовые fallback-поля + таблица SKU по цветам); панели табов скрыты условным display-классом (атрибут `hidden` проигрывает Tailwind `flex` — не возвращать). **Источник правды — форма: `upsertSkus` пишет `sku.marketplaces ?? {}` (полная замена), product-mapper ОБЯЗАН прокидывать `sku.marketplaces` — иначе сохранение молча трёт ссылки (регрессионный e2e в admin-marketplaces.spec).** Наполнение: `scripts/extract-marketplace-links.ts` (локально, CSV из gitignored `internal/content-source/` → коммитимый `src/core/db/marketplace-links.json`, 109 артикулов) → `npm run db:seed-mp-links` / tools-сервис `seed-marketplace-links` (idempotent, UPDATE по `skus.article`). После полного `test:e2e` сидер перезапускать (afterAll-`db:seed` трёт ссылки). **zod-грабли:** `z.record(z.enum,…)` в v4 exhaustive — для частичных мап использовать `z.partialRecord`.
