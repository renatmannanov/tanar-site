...
- [x] Нехватка: ошибка с «нужно N, доступно M», селект откатился, БД не изменилась (e2e)
- [x] После пополнения остатка подтверждение проходит (e2e)
- [x] Диалог удаления confirmed-заказа содержит «Резерв будет снят» (e2e в afterAll)
- [x] typecheck, lint, build — exit 0; admin-orders 7/7 (полный test:e2e — шаг 7)
