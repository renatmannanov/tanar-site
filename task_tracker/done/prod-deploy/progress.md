...
### 2026-06-04 — шаг 7 (деплой) + шаг 8 (бэкапы) выполнены
- **Демо ЖИВОЕ:** https://62-238-31-95.sslip.io (Hetzner 62.238.31.95, `~/tanar-site`, юзер rm_agent, ключ `~/.ssh/openclaw_hetzner`). LE-cert валиден. Сосед `wndrverse-db` (pgvector:5434) не мешает. Docker был предустановлен. ufw: открыты 80/443 (22 нетронут).
- **sslip.io** — времянка для демо (резолвит IP→домен, LE выдаёт cert). Боевой `.kz` будет на PS.kz (шаг 11).
- **Грабля: осиротевшие демо-фото.** В репо в `public/images/products/` лежали старые ИИ-демо-фото (5 папок: hoodie-alatau/turgen, light-jacket-tengri, shell-jacket-khan, tshirt-tanar — 153 webp). `Dockerfile COPY public` тащил их в образ → при первом `up` Docker сидит ими ПУСТОЙ volume → 153 файла в volume, но `media_assets=0` (на витрине НЕ видны, всё на градиентах). **ФИКС:** `public/images/products` добавлен в `.dockerignore`; образ пересобран; volume очищен (`docker run -v vol alpine rm -rf /data/*`). Витрина осталась 200 на градиентах. **Урок для шага 9/11:** slug'и тех фото (shell-jacket-khan и пр.) НЕ совпадают с товарами сида (jacket-sv7-goretex и пр.) — это разные наборы. Реальные фото поедут осознанно через push-media (шаг 9).
- **Бэкап (шаг 8):** `scripts/backup.sh` (pg_dump|gzip + tar volume, одна метка TS, ротация 14д, tar от host-user через `-u $(id -u):$(id -g)` чтобы ротация могла удалять) + `scripts/restore.sh` (деструктивный, требует "yes"). cron 3:30 ночью (рядом с openclaw-snapshot cron, его не трогали). Тест-restore дампа в изолированный postgres → 12/30/109, sentinel price 80000 ✓.
- **Грабля git: pull на сервере прерывался** — scp-копии `scripts/*.sh` (untracked) конфликтовали с теми же файлами, ставшими tracked в новом коммите. Решение: `rm` untracked-копий (сверив что git-версия не хуже) → `git pull`. На будущее: scp-файлы, которые потом коммитятся, удалять перед pull.
- **Грабля git: main↔dev структурно разошлись** (main = merge-коммит, не в линии dev) — ff невозможен. Каждый раз привожу main к дереву dev через `git merge --no-ff -s ours --no-commit dev && git read-tree -m -u dev && git commit`. Стоит занести в бэклог «выровнять историю dev/main» (для шага 10).

### 2026-06-05 — выкатка site-content + photo-pipeline (Фаза 1.5 → прод)
- **Выкачено:** merge dev→main (24 коммита: блог-статьи, /contacts, /faq из БД, core/site, photo-pipeline, seed-site). Прод: https://62-238-31-95.sslip.io — все страницы 200, контакты/FAQ/футер из БД, блог = 3 статьи, старые → 404, /admin/settings+/admin/faq за requireAdmin (307).
- **⚠️ ГЛАВНАЯ ГРАБЛЯ: Docker build cache отдаёт СТАРЫЙ `COPY . .`.** `up -d --build` пересобрал образ, но слой `COPY . .` взялся из кэша → в образе остались только миграции 0000–0001 (хотя на диске сервера все 5 после `git pull`), и migrate отрапортовал «applied successfully» НЕ применив 0002–0004 (таблиц site_settings/faq_items не было). Тот же кэш мог дать и старый код приложения в `web`. **ФИКС:** `docker compose build --no-cache <service>` для migrate, seed-site И web → пересобрать, затем migrate/seed/up заново. **Урок: при деплое, меняющем миграции/код, собирать `--no-cache` (или хотя бы для tools+web), не доверять `--build`.** Проверка образа: `run --rm --entrypoint sh <svc> -c 'ls /app/src/core/db/migrations/*.sql'` — должно показать ВСЕ файлы.
- **site_settings/faq на проде — через `seed-site` (не `seed`).** `seed.ts` блокируется count(products)>0 guard'ом (каталог уже засижен, 12 товаров). Новый idempotent `seed-site.ts` (+ compose-сервис `seed-site`, profile tools) сидит только site/faq, без ALLOW_PROD_SEED. Запуск: `--profile tools run --rm seed-site`.
- **GEMINI_API_KEY** добавлен в прод `.env` (взят из локального `.env.local`, бэкап `.env.bak.*` создан, права 600) — нужен для кнопок ИИ-генерации фото (приехал photo-pipeline). В git не попадает.
- **Каталог НЕ трогали** — 12 товаров остались, повторный сид заблокирован guard'ом (правильно). Фото товаров на проде по-прежнему нет (шаг 9, push-media).
- Безобидное в логах: `Failed to update prerender cache for /blog/<старый-slug> EACCES` — Next пытается кэшировать 404 удалённых статей при заходе на них; на работу не влияет.
- **⚠️ ГРАБЛЯ: Next standalone НЕ отдаёт рантайм-загрузки из public/.** Заказчица загрузила фото в админке прода → файл лёг на volume (`/app/public/images/products/<slug>/...webp`, права node:node ✓), строка в `media_assets` ✓, но `GET /images/products/...webp` → **404**. Причина: standalone-сервер обслуживает `public/` только для файлов, существовавших на момент билда; рантайм-загрузки он не видит как статику (локально не воспроизводится — dev отдаёт public/ динамически). **ФИКС:** Caddy отдаёт `/images/products/*` напрямую с того же volume (`handle_path /images/products/* { root * /srv/product-images; file_server }` + volume `product-images:/srv/product-images:ro` в caddy), остальное → reverse_proxy web. Применение: `git pull` → `up -d caddy` (recreate — новый volume mount, не restart). Пересборка web НЕ нужна. Проверка: `curl -I https://.../images/products/<slug>/<uuid>-1600.webp` → 200 image/webp. **Важно для шага 9 (push-media) и переезда на PS.kz — это решение там тоже нужно.**
- **⚠️ ГРАБЛЯ: `.env` НЕ пробрасывается в контейнер автоматически.** ИИ-генерация фото в админке падала «GEMINI_API_KEY не задан», хотя ключ был в прод `.env`. Причина: `--env-file .env` подгружает переменные только для подстановки `${...}` в самом compose-файле; в контейнер попадают ТОЛЬКО переменные, явно перечисленные в `environment:` сервиса. У `web` там были лишь DATABASE_URL/ADMIN_*. **ФИКС:** добавил `GEMINI_API_KEY: ${GEMINI_API_KEY}` (+опц. PHOTOGEN_*) в `environment:` web. Применение: `git pull` → `up -d web` (recreate, образ НЕ пересобирать). Проверка: `exec web sh -c 'echo $GEMINI_API_KEY'`. **Урок: любая новая env-переменная для рантайма = строка и в `.env`, И в `environment:` нужного сервиса.**
---
