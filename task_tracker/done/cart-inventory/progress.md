...
- **Комбо-переход (reserved→sold) пишет 2 строки журнала одной транзакцией
  с одинаковым created_at** — порядок строк внутри перехода не детерминирован,
  ассертить только состав (reason+delta), не порядок внутри пары.
- **ПРОД-ДЕПЛОЙ этого плана: миграций НЕТ, только код** —
  `docker compose ... up -d --build` (с `--no-cache` при сомнениях в кэше).
  Остатки на проде уже заведены (96/109 SKU) — механика включается сразу,
  отдельных действий заказчицы не требуется. Smoke после деплоя: карточка
  с точкой наличия → распроданный размер → подтверждение заказа в админке.
