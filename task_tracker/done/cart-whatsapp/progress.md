...
- **zod v4: `z.record(z.enum(...), ...)` — exhaustive.** Латентный баг в
  `repository.ts:303`: схема требовала ОБА ключа (kaspi И ozon), сохранить
  товар с одной МП-ссылкой было невозможно (падало «Invalid input: expected
  string, received undefined»). До шага 8 не стреляло — UI-полей не было, сид
  без marketplaces. Фикс: `z.partialRecord` (шаг 8).
- Drawer открывается после «В корзину» → e2e, кликающий «В корзину» дважды,
  должен закрывать drawer (Escape) между кликами — иначе overlay перехватит клик.
- `git add -A src` затащил чужие рабочие файлы `src/photo/` в коммит шага 2 —
  убраны amend'ом; добавлять файлы точечно.
